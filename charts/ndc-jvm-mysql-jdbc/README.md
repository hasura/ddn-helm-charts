# Ndc-jvm-mysql-jdbc Helm Chart

This chart deploys the ndc-jvm-mysql-jdbc connector (For use under PromptQL use cases). Refer to the pre-requisites section [here](../../README.md#get-started)
...
helm template <release-name> \
  ...
  --set image.repository="my_repo/ndc-jvm-mysql-jdbc" \
  --set image.tag="my_custom_image_tag" \
  --set connectorEnvVars.JDBC_URL="jdbc_url" \
  --set connectorEnvVars.HASURA_SERVICE_TOKEN_SECRET="token" \
  hasura-ddn/ndc-jvm-mysql-jdbc | kubectl apply -f-
...
| `image.repository`                                | Image repository containing custom created ndc-jvm-mysql-jdbc                                                    | `""`                                |
| `image.tag`                                       | Image tag to use for custom created ndc-jvm-mysql-jdbc                                                           | `""`                                |
...
| `extraVolumes`                                    | Optionally specify extra list of additional volumes for the ndc-jvm-mysql-jdbc pod                                | `[]`                                |
| `extraContainers`                                 | Optionally specify extra list of additional containers for the ndc-jvm-mysql-jdbc pod                             | `[]`                                |
| `resources`                                       | Resource requests and limits of ndc-jvm-mysql-jdbc container                                                      | `{}`                                |
| `env`                                             | Env variable section for ndc-jvm-mysql-jdbc                                                                      | `[]`                                |
| `replicas`                                        | Replicas setting for pod                                                                                   | `1`                                 |
| `wsInactiveExpiryMins`                            | To be documented                                                                                           | `1`                                 |
| `securityContext`                                 | Define privilege and access control settings for a Pod or Container                                        | `{}`                                |
| `healthChecks.enabled`                            | Enable health check for ndc-jvm-mysql-jdbc container                                                              | `false`                             |
| `healthChecks.livenessProbePath`                  | Health check liveness Probe path ndc-jvm-mysql-jdbc container                                                     | `"/healthz"`                        |
| `healthChecks.readinessProbePath`                 | Health check readiness Probe path mongo-connector container                                                | `"/healthz"`                        |
| `hpa.enabled`                                     | Enable HPA for ndc-jvm-mysql-jdbc.  Ensure metrics cluster is configured when enabling                            | `false`                             |
