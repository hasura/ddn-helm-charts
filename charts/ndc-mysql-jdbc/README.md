# Ndc-mysql-jdbc Helm Chart

This chart deploys the ndc-mysql-jdbc connector (For use under PromptQL use cases). Refer to the pre-requisites section [here](../../README.md#get-started)

## Connector Image

If you're running `docker compose build` within your Supergraph to build a custom connector image, or if you're using
the **git-sync** option with a Hasura-provided connector image, the base image used in both cases is: `ghcr.io/hasura/ndc-mysql-jdbc`
...
helm template <release-name> \
  ...
  --set image.repository="my_repo/ndc-mysql-jdbc" \
  --set image.tag="my_custom_image_tag" \
  --set connectorEnvVars.JDBC_URL="jdbc_url" \
  --set connectorEnvVars.HASURA_SERVICE_TOKEN_SECRET="token" \
  hasura-ddn/ndc-mysql-jdbc | kubectl apply -f-
...
| `healthChecks.enabled`                            | Enable health check for ndc-mysql-jdbc container                                                              | `false`                             |
| `healthChecks.livenessProbePath`                  | Health check liveness Probe path ndc-mysql-jdbc container                                                     | `"/healthz"`                        |
| `healthChecks.readinessProbePath`                 | Health check readiness Probe path mongo-connector container                                                | `"/healthz"`                        |
| `hpa.enabled`                                     | Enable HPA for ndc-mysql-jdbc.  Ensure metrics cluster is configured when enabling                            | `false`                             |
