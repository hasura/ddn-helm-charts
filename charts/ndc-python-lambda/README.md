# Ndc-python-lambda Helm Chart

This chart deploys the ndc-python-lambda connector. Refer to the pre-requisites section [here](../../README.md#get-started)

## Connector Image

If you're running `docker compose build` within your Supergraph to build a custom connector image, or if you're using
the **git-sync** option with a Hasura-provided connector image, the base image used in both cases is: `ghcr.io/hasura/ndc-python-lambda`
...
helm template <release-name> \
  ...
  --set image.repository="my_repo/ndc-python-lambda" \
  --set image.tag="my_custom_image_tag" \
  --set connectorEnvVars.HASURA_SERVICE_TOKEN_SECRET="token" \
  hasura-ddn/ndc-python-lambda | kubectl apply -f-
...
By **default**, the setup expects the `venv` directory (produced by running `pip install`) to be committed and present in the Git repository.

If you prefer **not to commit** the `venv` directory to your repository, you can configure the system to use pre-packaged dependencies from a Docker image instead. To do this:

- Build a custom Docker image that includes the `venv` directory.
...
| `connectorEnvVars.OTEL_SERVICE_NAME`              | OTEL Service Name (Optional)                                                                               | `ndc-python-lambda`                  |
...
| `image.repository`                                | Image repository containing custom created ndc-python-lambda                                                    | `""`                            |
| `image.tag`                                       | Image tag to use for custom created ndc-python-lambda                                                           | `""`                            |
| `image.pullPolicy`                                | Image pull policy                                                                                          | `Always`                        |
| `resources`                                       | Resource requests and limits of ndc-python-lambda container                                                     | `{}`                            |
| `env`                                             | Env variable section for ndc-python-lambda                                                                      | `[]`                            |
| `replicas`                                        | Replicas setting for pod                                                                                   | `1`                             |
| `wsInactiveExpiryMins`                            | To be documented                                                                                           | `1`                             |
| `securityContext`                                 | Define privilege and access control settings for a Pod or Container                                        | `{}`                            |
| `healthChecks.enabled`                            | Enable health check for ndc-python-lambda container                                                             | `false`                         |
| `healthChecks.livenessProbePath`                  | Health check liveness Probe path ndc-python-lambda container                                                    | `"/healthz"`                    |
| `healthChecks.readinessProbePath`                 | Health check readiness Probe path ndc-python-lambda container                                                   | `"/healthz"`                    |
| `hpa.enabled`                                     | Enable HPA for ndc-python-lambda.  Ensure metrics cluster is configured when enabling                           | `false`                         |
...
| `initContainers.gitSync.depsPackaged`             | Whether venv was packaged into git repo (Used when initContainers.gitSync.enabled is set to true)          | `true`                              |